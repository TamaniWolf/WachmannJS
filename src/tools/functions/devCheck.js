...
class DevCheck {
    ...
    static BotMaster(message) {
        return new Promise((resolve, reject) => {
            try {
                ...
                const serverOwner = serverOwnerArray.filter(obj => obj === msgAuthor);
                const botMaster = botMasterArray.filter(obj => obj === msgAuthor);
    ...
    static BotMasterRole(message) {
        return new Promise(async (resolve, reject) => {
            try {
                let masterRole;
                let guild = await message.client.guilds.fetch(process.env.SERVER_ID);
                let member = await guild.members.fetch(message.author.id);
                let role = await member.roles.cache.get(process.env.BOT_MASTER_ROLE);
                if (role != null) {
                    masterRole = true;
                };
    ...
    static BotChannel(message) {
        return new Promise((resolve, reject) => {
            try {
                ...
                const botChannel = botChannelArray.filter(obj => obj === msgChannel);
    ...
    static LogChannel() {
        return new Promise((resolve, reject) => {
            try {
                ...
                const firstChannel = logChannelArray[0];
