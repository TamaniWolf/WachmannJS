...
module.exports = {
	...
	description: "Log Stage Instance Delete.",
	once: false,
	async execute(stageInstance) {
		// console.log("stageInstanceDelete.js");
		...
		if (targetType !== "StageInstance" && actionType !== "Delete") return;

		let icon2 = executor.avatarURL();
		if (executor.avatar == null) icon2 = "https://i.imgur.com/CN6k8gB.png";
		const timestamp = DateTime.utc().toUnixInteger();
		const stageDelete = new EmbedBuilder()
			.setAuthor({ name: `${executor.tag} (ID: ${executor.id})`, iconURL: `${icon2}` })
			.setColor(Application.colors().logEmbedColor.create)
			.setDescription(LanguageConvert.lang(langLogs.voice.stageend, target.channelId, timestamp))
			.addFields(
				...
				{ name: langLogs.voice.discoverable, value: `${target.discoverableDisabled}`, inline: true },
				{ name: langLogs.voice.eventid, value: `${target.guildScheduledEventId}`, inline: true }
			)
			.setFooter({ text: "StageInstanceCreate" })
			.setTimestamp(new Date());
		// eslint-disable-next-line no-undef
		globalclient.channels.cache.get(logChannel).send({ embeds: [stageDelete] });
	}
};