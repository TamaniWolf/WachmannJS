...
module.exports = {
	...
	async execute(oldThread, newThread) {
		...
		const logChannel = await DevCheck.LogChannel(newThread.guild.id);
		if (logChannel === "0") return;
		const { Application } = require("../../core/application/Application.js");
		const { DateTime } = require("luxon");
		// SQLite
		const { Get, Set, Del } = require("../../tools/functions/sqlite/prepare.js");
		// AuditLog Fetch
		...
		// 	culogs = threadUpdateLog.createdTimestamp;
		// }
		// let coulogs = 2;
		...
		// if (culogs > coulogs) {
		if (threadUpdateLog != null) {
			dataAuditLogIDCU = Get.auditLogs(threadUpdateLog.id);
		}
		...
		if (dataAuditLogIDCU == null || id !== dataAuditLogIDCU.AuditLogID) {
			if (targetType === "Thread" && actionType === "Update") {
				...
				if (changeLocked && changeLocked.length !== 0) { cuo += `**${lang.logs.locked}:** ${changeLocked[0].old}\n`; }
				...
				if (changeArchived && changeArchived.length !== 0) { cuo += `**${lang.logs.archived}:** ${changeArchived[0].old}\n`; }
				...
				if (changeLocked && changeLocked.length !== 0) { cun += `**${lang.logs.locked}:** ${changeLocked[0].new}\n`; }
				if (changeType && changeType.length !== 0) {
					const chaType = await MiscConvert.channelTypeName(changeType[0].new);
					cun += `**${lang.logs.type}:** ${chaType}\n`;
				}
				if (changeArchived && changeArchived.length !== 0) { cun += `**${lang.logs.archived}:** ${changeArchived[0].new}\n`; }
				if (changeRLPU && changeRLPU.length !== 0) {
				...
				if (dataAuditLogIDCU == null) {
					...
					if (executor.bot === true) {
						...
						dataAuditLogIDCU = { AuditLogID: `${id}`, GuildID: `${newThread.guild.id}`, Type: "Thread_Update", Date: `${threadUpdateLog.createdTimestamp}` };
		...
		// 				dataAuditLogIDCOU = { AuditLogID: `${id}`, GuildID: `${newThread.guild.id}`, Type: "Thread_Update", Date: `${channelOverwriteUpdate.createdTimestamp}` };
		...
		dataAuditLogDate = Get.allAuditLogs("Thread_Update");
