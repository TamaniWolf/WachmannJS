            ...
            if (botMasterRole === true || botMaster === true) {
                if (botChannel === true) {
                        ...
                        if (message.content.startsWith(`${tellMeAbout} <@`) && !botAliasis4) {
                            let stringChannelId = args[2].toString();
                            const stringGetUser = stringChannelId.replace(/[/</>/@]/g, '');
                            if (stringGetUser) {
                                let guild = message.client.guilds.cache.get(getGuildID);
                                let memberTagged = await guild.members.fetch(stringGetUser);
                        ...
                        if (message.content.startsWith(`${tellMeAbout} <#`)) {
                            ...
                            let category = guild.channels.cache.get(channel.parentId);
